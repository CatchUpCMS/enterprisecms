...
{{-- Web site Title --}}
@section('title')
{{ Lang::choice('core::cms.link', 2) }} :: @parent
@stop
...
<h1>
	...
	<a href="/admin/menulinks/create/{{ $create_id }}" class="btn btn-primary" title="{{ trans('core::button.new') }}">
		<i class="fa fa-plus fa-fw"></i>
		{{ trans('core::button.new') }}
	</a>
	</p>
	<i class="fa fa-chain fa-lg"></i>
	{{ Lang::choice('core::cms.link', 2) }}
	<hr>
</h1>
</div>
...
<div role="tabpanel" class="tab-pane padding fade @if ($language->locale == $lang)in active @endif" id="lang_{{{ $language->id }}}">
	...
	<table id="table" class="table table-striped table-hover">
		<thead>
			<tr>
				<th>{{ trans('core::table.title') }}</th>
				<th>{{ trans('core::table.url') }}</th>
				<th>{{ trans('core::table.position') }}</th>
				<th>{{ trans('core::table.menu') }}</th>
		...
		<tbody>
			@foreach ($links as $link)
				<tr>
					...
					<td>
						<a href="/admin/menulinks/{{ $link->id }}/edit" class="btn btn-success" title="{{ trans('core::button.edit') }}">
